...
static int aw8896_spk_control = 0;
static int aw8896_rcv_control = 0;
        ...
        struct aw8896_container *aw8896_cont, int base)
{
    ...
    for(i=0; i<aw8896_cont->len; i+=MAX_RAM_WRITE_BYTE_SIZE) {
        ...
        aw8896_i2c_writes(aw8896, AW8896_REG_DSPMDAT, &aw8896_cont->data[i], tmp_len);
    }
#else
    /* i2c write */
    for(i=0; i<aw8896_cont->len; i+=2) {
...
//    int i;
    ...
    if (!cont) {
        pr_err("%s: failed to read %s\n", __func__, aw8896_fw_name[aw8896->dsp_fw_ver]);
        release_firmware(cont);
        return;
    }
...
static int aw8896_load_fw(struct aw8896 *aw8896)
{
    ...
    return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
                                aw8896_fw_name[aw8896->dsp_fw_ver], aw8896->dev, GFP_KERNEL,
...
static int aw8896_volume_info(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_info *uinfo)
{
...
static int aw8896_volume_get(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol)
{
...
static int aw8896_volume_put(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol)
{
    struct soc_mixer_control *mc = (struct soc_mixer_control*) kcontrol->private_value;
    struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
    struct aw8896 *aw8896 = snd_soc_codec_get_drvdata(codec);
...
{
    //struct aw8896 *aw8896 = snd_soc_codec_get_drvdata(dai->codec);
    struct snd_soc_codec *codec = dai->codec;
    ...
    struct snd_soc_dai *dai)
{
    ...
    if(-1 != reg_value){
        ...
        reg_value = reg_value | (tmp_value& (~AW8896_BIT_I2SCTRL_SR_MASK));
    ...
    if(-1 != reg_value){
        aw8896_i2c_read(aw8896,AW8896_REG_I2SCTRL,&tmp_value);
        reg_value = reg_value | (tmp_value&(~AW8896_BIT_I2SCTRL_FMS_MASK));
        aw8896_i2c_write(aw8896,AW8896_REG_I2SCTRL,reg_value);
    }
    mutex_unlock(&aw8896->lock);
...
static int aw8896_remove(struct snd_soc_codec *codec)
{
    ...
    //aw8896_inputdev_unregister(aw8896);
...
static unsigned int aw8896_codec_read(struct snd_soc_codec *codec,unsigned int reg)
{
    struct aw8896 *aw8896=snd_soc_codec_get_drvdata(codec);
...
static int aw8896_codec_write(struct snd_soc_codec *codec,unsigned int reg,unsigned int value)
{
    int ret ;
    struct aw8896 *aw8896=snd_soc_codec_get_drvdata(codec);
...
/*
static int aw8896_codec_readable(struct snd_soc_codec *codec,unsigned int reg)
{
    return aw8896_reg_access[reg]&REG_RD_ACCESS;
}
*/
...
int aw8896_read_chipid(struct aw8896 *aw8896)
{
    ...
    while(cnt < AW_READ_CHIPID_RETRIES) {
        ...
        if (ret < 0) {
            dev_err(aw8896->dev, "%s: failed to read register AW8896_REG_ID: %d\n", __func__, ret);
            return -EIO;
        }
        switch (reg) {
        case 0x0310:
 ...
 *
 ******************************************************/
static ssize_t aw8896_reg_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t aw8896_reg_show(struct device *dev, struct device_attribute *attr,
                char *buf)
{
    struct aw8896 *aw8896 = dev_get_drvdata(dev);
    ssize_t len = 0;
...
static ssize_t aw8896_dsp_store(struct device *dev, struct device_attribute *attr,
                  const char *buf, size_t count)
{
    struct aw8896 *aw8896 = dev_get_drvdata(dev);
    ...
    if(1 == sscanf(buf, "%d", &databuf[0])) {
        if(databuf[0] == 1) {
            ...
            if(reg_val & AW8896_BIT_SYSST_PLLS) {
                ...
                count += snprintf((char *)(buf+count), PAGE_SIZE-count, "aw8896 plls=%d, no iis signal\n",
...
static ssize_t aw8896_dsp_show(struct device *dev, struct device_attribute *attr,
                char *buf)
{
    struct aw8896 *aw8896 = dev_get_drvdata(dev);
    ssize_t len = 0;
    ...
    if(reg_val & AW8896_BIT_SYSCTRL_DSPBY) {
        ...
        aw8896_i2c_read(aw8896, AW8896_REG_SYSST, &reg_val);
        if(reg_val & AW8896_BIT_SYSST_PLLS) {
            len += snprintf(buf+len, PAGE_SIZE-len, "aw8896 dsp working\n");

            aw8896_dsp_enable(aw8896, false);

            len += snprintf(buf+len, PAGE_SIZE-len, "aw8896 dsp firmware:\n");
            addr = 0;
            for(i=0; i<aw8896->dsp_fw_len; i+=2) {
                aw8896_i2c_write(aw8896, AW8896_REG_DSPMADD, AW8896_DSP_FW_BASE+addr);
                aw8896_i2c_read(aw8896, AW8896_REG_DSPMDAT, &reg_val);
            ...
            len += snprintf(buf+len, PAGE_SIZE-len, "aw8896 dsp config:\n");
            addr = 0;
            for(i=0; i<aw8896->dsp_cfg_len; i+=2) {
                aw8896_i2c_write(aw8896, AW8896_REG_DSPMADD, AW8896_DSP_CFG_BASE+addr);
                aw8896_i2c_read(aw8896, AW8896_REG_DSPMDAT, &reg_val);
                len += snprintf(buf+len, PAGE_SIZE-len, "0x%02x,0x%02x,", reg_val&0x00FF, (reg_val>>8));
                if((i/2+1)%4 == 0)
                     len += snprintf(buf+len, PAGE_SIZE-len, "\n");
            ...
            aw8896_dsp_enable(aw8896, true);
                ...
                len += snprintf((char *)(buf+len), PAGE_SIZE-len, "aw8896 plls=%d, no iis signal\n",
                        reg_val&AW8896_BIT_SYSST_PLLS);
        }
    }
...
static ssize_t aw8896_spk_rcv_store(struct device *dev, struct device_attribute *attr,
                  const char *buf, size_t count)
{
    struct aw8896 *aw8896 = dev_get_drvdata(dev);
...
static ssize_t aw8896_spk_rcv_show(struct device *dev, struct device_attribute *attr,
                char *buf)
{
    struct aw8896 *aw8896 = dev_get_drvdata(dev);
    ssize_t len = 0;
    if(aw8896->spk_rcv_mode == AW8896_SPEAKER_MODE) {
        ...
        len += snprintf(buf+len, PAGE_SIZE-len, "aw8896 spk_rcv: %d, receiver mode\n", aw8896->spk_rcv_mode);
    } else {
        len += snprintf(buf+len, PAGE_SIZE-len, "aw8896 spk_rcv: %d, unknown mode\n", aw8896->spk_rcv_mode);
    }

    return len;
}

static DEVICE_ATTR(reg, S_IWUSR | S_IRUGO, aw8896_reg_show, aw8896_reg_store);
static DEVICE_ATTR(dsp, S_IWUSR | S_IRUGO, aw8896_dsp_show, aw8896_dsp_store);
static DEVICE_ATTR(spk_rcv, S_IWUSR | S_IRUGO, aw8896_spk_rcv_show, aw8896_spk_rcv_store);
 ...
 *
 ******************************************************/
static int aw8896_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
...
static struct of_device_id aw8896_dt_match[] = {
